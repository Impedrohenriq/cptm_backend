-- =====================================================================
-- V0__create_database_full.sql
-- Script unico para criacao completa do banco/modelagem do zero
-- Nao depende de nenhum outro script
-- =====================================================================
-- Uso (SQLcl/SQL*Plus):
--   SQL> @V0__create_database_full.sql
--
-- Parametro opcional:
--   DEFINE V0_LOAD_SAMPLE_DATA = N   -> cria somente estrutura (padrao)
--   DEFINE V0_LOAD_SAMPLE_DATA = Y   -> cria estrutura + dados de exemplo/teste

SET DEFINE ON
DEFINE V0_LOAD_SAMPLE_DATA = N

PROMPT [1/3] Criando estrutura base...
-- =====================================================================
...
USING (
  SELECT 'EEA.EF-A.2026-L.07-CPTM-N.000999' AS CHAVE_PRIMARIA_MA
    FROM DUAL
   WHERE UPPER('&V0_LOAD_SAMPLE_DATA') = 'Y'
) s
ON (t.CHAVE_PRIMARIA_MA = s.CHAVE_PRIMARIA_MA)
WHEN MATCHED THEN UPDATE SET
  t.NR_ELEMENTO_MONIT = '000999',
  t.NM_ELEMENTO_MONIT = 'Exemplo Ponto de Monitoramento',
  t.NM_CONTRATADA = 'Companhia Paulista de Trens Metropolitanos - CPTM',
  t.NM_AUTOR_CADASTRAMENTO = 'Joao Inspetor',
  t.NM_RESPONSAVEL_TECNICO = 'Eng. Tecnico Exemplo',
  t.DT_EMISSAO_FORMULARIO = TO_DATE('2026-04-17','YYYY-MM-DD'),
  t.NR_FORMULARIO = '000999',
  t.DT_CADASTRAMENTO = TO_DATE('2026-04-17','YYYY-MM-DD'),
...
WHEN NOT MATCHED THEN INSERT (
  ...
  NM_ELEMENTO_MONIT,
  NM_CONTRATADA,
  NM_AUTOR_CADASTRAMENTO,
  NM_RESPONSAVEL_TECNICO,
  DT_EMISSAO_FORMULARIO,
  NR_FORMULARIO,
  DT_CADASTRAMENTO,
  NM_MUNICIPIO,
  NM_LINHA_CPTM,
  NM_ESTACAO_CPTM,
  DS_OBSERVACOES_CADASTRO
) VALUES (
  'EEA.EF-A.2026-L.07-CPTM-N.000999',
  '000999',
  'Exemplo Ponto de Monitoramento',
...
USING (
  SELECT 'EEA.EF-A.2026-L.07-CPTM-N.000999' AS CHAVE_PRIMARIA_MA, 1 AS NR_FOTO
    FROM DUAL
   WHERE UPPER('&V0_LOAD_SAMPLE_DATA') = 'Y'
) s
...
DELETE FROM TB_FDC_EEA_EF_FOTO
 WHERE CHAVE_PRIMARIA_MA = 'EEA.EF-A.2026-L.08-CPTM-N.009998'
   AND UPPER('&V0_LOAD_SAMPLE_DATA') = 'Y';

DELETE FROM TB_FDC_EEA_EF
 WHERE CHAVE_PRIMARIA_MA = 'EEA.EF-A.2026-L.08-CPTM-N.009998'
   AND UPPER('&V0_LOAD_SAMPLE_DATA') = 'Y';

INSERT INTO TB_FDC_EEA_EF (
    CHAVE_PRIMARIA_MA,
    NR_ELEMENTO_MONIT,
    NM_ELEMENTO_MONIT,
    NM_CONTRATADA,
...
PROMPT Validacao (sai linha apenas quando V0_LOAD_SAMPLE_DATA=Y)...
SELECT
  f.CHAVE_PRIMARIA_MA,
...
WHERE f.CHAVE_PRIMARIA_MA = 'EEA.EF-A.2026-L.08-CPTM-N.009998'
  AND UPPER('&V0_LOAD_SAMPLE_DATA') = 'Y'
GROUP BY f.CHAVE_PRIMARIA_MA;

PROMPT Concluido com sucesso.
